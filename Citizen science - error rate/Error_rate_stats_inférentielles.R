...
model1<-lmer(log(MR_EPI+1) ~ Nbr_EW + Div_LIEC+ ADJV + nuages +(1|ID_Site),data = SBT_error_MREPI)
...
model1<-lmer(log(MR_EPI + 1) ~ Div_LIEC + ADJV + (1 | ID_Site),data = SBT_error_MREPI)
...
model1<-lmer(MR_ANE_RH ~ Nbr_EW + Div_LIEC+ ADJV + nuages +(1|ID_Site),data = SBT_error_MRANE_RH)
...
bartlett.test(MR_ANE_RH~ID_Site,data=SBT_error_MRANE_RH)
...
model1<-lmer(MR_ANE_BH ~ Nbr_EW + Div_LIEC+ ADJV + nuages +(1|ID_Site),data = SBT_error_MRANE_BH)
...
model1<-lmer(MR_ANE_BH ~ Div_LIEC + ADJV + (1 | ID_Site),data = SBT_error_MRANE_BH)
Anova(model1,type=2)
MuMIn::r.squaredGLMM(model1)
...
model1<-lmer(MR_END ~ Nbr_EW + Div_LIEC+ ADJV + nuages +(1|ID_Site),data = SBT_error_MREND)
model1<-glmmTMB(MR_rounded ~ EC* (Nbr_EW + Div_LIEC+ ADJV + nuages) +(1|ID_Site),data = SBT_error_MR, family= "poisson")
...
model1<-lmer(MR_END ~ Nbr_EW + ADJV + (1 | ID_Site),data = SBT_error_MREND)
Anova(model1,type=2)
MuMIn::r.squaredGLMM(model1)
...
model1<-lmer(UR_EPI ~ Nbr_EW + Div_LIEC+ ADJV + nuages +(1|ID_Site),data = SBT_error_UREPI)
...
model1<-lmer(log(UR_EPI + 1) ~ Div_LIEC + ADJV + (1 | ID_Site),data = SBT_error_UREPI)
#refaire tableau unique (une seule colonne UR + UR)
MuMIn::r.squaredGLMM(model1)
...
model1<-lmer(UR_rounded ~ Div_LIEC + nuages + (1 | ID_Site),data = SBT_error_UREND)
Anova(model1,type=2)
MuMIn::r.squaredGLMM(model1)
